...
export default function Tiffin() {
  ...
  const toggleFavorite = useCallback(async (tiffinId, title) => {
    ...
    setFavoriteServices(prev =>
      prev.includes(tiffinId)
        ? prev.filter(id => id !== tiffinId)
        : [...prev, tiffinId]
    );
    ...
    try {
      ...
      //   Alert.alert('Added to favorites', `${title} has been added to your favorites`);
      // } else {
      //   Alert.alert('Removed from favorites', `${title} has been removed from your favorites`);
      // }
    } catch (error) {
  ...
  useCallback(() => {
    const fetchLikedTiffins = async (tiffinId) => {
      if (isAuthenticated && user?.id) {
        try {
          const response = await axios.get(`${Api_url}/api/tiffins/${tiffinId}/like`, {
            headers: { "Content-Type": "application/json" },
            withCredentials: true
          });
          const likedTiffins = response.data.likedTiffins || [];
          setFavoriteServices(likedTiffins.map(t => t._id));
        } catch (error) {
...
// useEffect(() => {
//   if (localTiffinData.length > 0) {
//     updateRecommendedTiffins(localTiffinData);
//   }
// }, [localTiffinData, updateRecommendedTiffins]);
...
console.log('rec',recommendedTiffins)
