...
const STORAGE_KEY = 'STATIC_PROFILE';

const ProfileScreen = () => {
  // ✅ only user (no updateUser)
  const { user } = useAuth();
  ...
  const handleUpdate = async () => {
    if (isEditing) {
      try {
        ...
        console.error('Static profile update error:', error);
            ...
            <View key={key} className="mb-4">
              ...
              <View className="border border-border rounded-lg px-3 py-2">
                {isEditing && key !== 'email' ? (
                  ...
                  <Text className="text-base text-textsecondary">{localProfile[key] || 'Not set'}</Text>
                )}
              </View>
            </View>
          ))}
